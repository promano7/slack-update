...
PKGROOT=$TMP/pkgroot
CACHE=$TMP/cache
mkdir -p "$PKGROOT/boot" "$PKGROOT/install" "$PKGROOT/lib/modules/6.18.41/kernel/drivers/test" "$CACHE/a"
printf 'kernel\n' > "$PKGROOT/boot/vmlinuz-6.18.41"
printf 'module\n' > "$PKGROOT/lib/modules/6.18.41/kernel/drivers/test/test.ko"
cat > "$PKGROOT/install/doinst.sh" <<'EOF_DOINST'
#!/bin/sh
if [ -r boot/vmlinuz-6.18.41 ]; then
...
assert_success 'the accepted candidate and boot records should match the exact transaction' validate_accepted_records
cp "$BOOT_PREFLIGHT" "$TMP/boot-mismatch.json"
sed -i 's/6.18.41/6.18.42/' "$TMP/boot-mismatch.json"
...
mkdir "$OUT"
PKGLIST="$TMP/pkglist"
printf 'slackware64 kernel-generic 6.18.41 x86_64 1 kernel-generic-6.18.41-x86_64-1 ./slackware64/a
' > "$PKGLIST"
assert_success 'one exact live repository record should validate' validate_live_repository_target "$PKGLIST" 6.18.41 "$OUT/live-record"
printf 'slackware64 kernel-generic 6.18.41 x86_64 1 kernel-generic-6.18.41-x86_64-1 ./slackware64/a
' >> "$PKGLIST"
assert_failure 'duplicate live repository records should fail closed' validate_live_repository_target "$PKGLIST" 6.18.41 "$OUT/live-record"
printf 'slackware64 kernel-generic 6.18.42 x86_64 1 kernel-generic-6.18.42-x86_64-1 ./slackware64/a
' > "$PKGLIST"
assert_failure 'a changed live target should fail closed' validate_live_repository_target "$PKGLIST" 6.18.41 "$OUT/live-record"

assert_success 'one exact cached package should resolve' resolve_exact_cached_package "$CACHE" 6.18.41 "$OUT/cached.txt"
assert_equal "$PACKAGE" "$(cat "$OUT/cached.txt")" 'the exact cached package path should be emitted'
cp "$PACKAGE" "$CACHE/kernel-generic-6.18.41-x86_64-1.txz"
assert_failure 'duplicate exact cached packages should fail closed' resolve_exact_cached_package "$CACHE" 6.18.41 "$OUT/duplicate.txt"
rm "$CACHE/kernel-generic-6.18.41-x86_64-1.txz"

assert_success 'a safe synthetic package should inspect' inspect_package_archive "$PACKAGE" 6.18.41 "$OUT/members" "$OUT/doinst" "$OUT/package-summary"
assert_contains 'boot/vmlinuz-6.18.41' "$OUT/members" 'the versioned kernel should be inventoried'
assert_contains 'lib/modules/6.18.41/kernel/drivers/test/test.ko' "$OUT/members" 'target modules should be inventoried'
...
rm -rf "$BADROOT/lib/modules/6.18.41"
BADPKG=$TMP/kernel-generic-6.18.41-x86_64-1.txz
( cd "$BADROOT" && tar -cJf "$BADPKG" . )
assert_failure 'a package without modules should fail closed' inspect_package_archive "$BADPKG" 6.18.41 "$OUT/bad-members" "$OUT/bad-doinst" "$OUT/bad-summary"
...
assert_failure 'foreign module versions should fail closed' inspect_package_archive "$BADPKG" 6.18.41 "$OUT/bad-members" "$OUT/bad-doinst" "$OUT/bad-summary"

cp -a "$PKGROOT" "$BADROOT.initrd"
printf initrd > "$BADROOT.initrd/boot/initrd.gz"
( cd "$BADROOT.initrd" && tar -cJf "$BADPKG" . )
assert_failure 'an embedded initrd should fail closed' inspect_package_archive "$BADPKG" 6.18.41 "$OUT/bad-members" "$OUT/bad-doinst" "$OUT/bad-summary"

MALICIOUS_DIR="$TMP/malicious"
mkdir -p "$MALICIOUS_DIR"
python3 - "$MALICIOUS_DIR/kernel-generic-6.18.41-x86_64-1.txz" <<'PY_TAR'
...
PY_TAR
assert_failure 'archive parent traversal should fail closed' inspect_package_archive "$MALICIOUS_DIR/kernel-generic-6.18.41-x86_64-1.txz" 6.18.41 "$OUT/bad-members" "$OUT/bad-doinst" "$OUT/bad-summary"
python3 - "$MALICIOUS_DIR/kernel-generic-6.18.41-x86_64-1.txz" <<'PY_TAR'
import tarfile, sys
with tarfile.open(sys.argv[1], 'w:xz') as archive:
    member = tarfile.TarInfo('lib/modules/6.18.41/escape')
    member.type = tarfile.SYMTYPE
    member.linkname = '../../../../etc/passwd'
    archive.addfile(member)
PY_TAR
assert_failure 'archive links escaping the package root should fail closed' inspect_package_archive "$MALICIOUS_DIR/kernel-generic-6.18.41-x86_64-1.txz" 6.18.41 "$OUT/bad-members" "$OUT/bad-doinst" "$OUT/bad-summary"
...
EOF_BAD
assert_failure 'doinst must not invoke mkinitrd' validate_doinst_policy "$OUT/doinst.bad" 6.18.41 "$OUT/bad-policy"
cat > "$OUT/doinst.bootloader" <<'EOF_BOOTLOADER'
#!/bin/sh
lilo
ln -sf vmlinuz-6.18.41 vmlinuz-generic
EOF_BOOTLOADER
assert_failure 'doinst must not invoke another boot loader' validate_doinst_policy "$OUT/doinst.bootloader" 6.18.41 "$OUT/bad-policy"
printf '%s\n' '#!/bin/sh' 'if then' 'ln -sf vmlinuz-6.18.41 vmlinuz-generic' > "$OUT/doinst.syntax"
assert_failure 'invalid doinst shell syntax should fail closed' validate_doinst_policy "$OUT/doinst.syntax" 6.18.41 "$OUT/bad-policy"
...
ln -sf vmlinuz-6.18.41 vmlinuz-generic
EOF_BAD
assert_failure 'duplicate generic transitions should fail closed' validate_doinst_policy "$OUT/doinst.bad" 6.18.41 "$OUT/bad-policy"
cat > "$OUT/doinst.bad" <<'EOF_BAD'
#!/bin/sh
ln -sf vmlinuz-6.18.40 vmlinuz-generic
EOF_BAD
assert_failure 'a transition to the wrong version should fail closed' validate_doinst_policy "$OUT/doinst.bad" 6.18.41 "$OUT/bad-policy"
cat > "$OUT/doinst.bad" <<'EOF_BAD'
#!/bin/sh
rm -rf /
ln -sf vmlinuz-6.18.41 vmlinuz-generic
