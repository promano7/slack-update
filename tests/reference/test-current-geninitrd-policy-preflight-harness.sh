...
assert_success 'the three accepted records should match the policy transaction' validate_accepted_records
cp "$PACKAGE_PREFLIGHT" "$TMP/package-mismatch.json"
sed -i 's/6.18.41/6.18.42/' "$TMP/package-mismatch.json"
...
assert_equal /boot/initrd-6.18.41.img "$(json_value "$OUT" expected_initrd)" 'the versioned initrd path should be predicted'
...
assert_contains 'conditional_geninitrd_hook' "$REPOSITORY_ROOT/tests/fixtures/reference/acceptance/kernel-boot/slackware-current-kernel-package-preflight-20260803-accepted.json" 'accepted package evidence should record the hook'
assert_contains 'host_policy_preflight_required' "$REPOSITORY_ROOT/tests/fixtures/reference/acceptance/kernel-boot/slackware-current-kernel-package-preflight-20260803-accepted.json" 'accepted package evidence should require this stage'
