...
ACCEPTED_REBIND="$FIXTURES/slackware-15.0-elilo-oldkernel-cleanup-recovery-backup-removal-boot-rebind-review-20260815-accepted.json"
...
CONFIRM_REBIND_EVIDENCE_SHA256=""
...
usage() {
    cat <<'USAGE'
Usage:
  test-elilo-oldkernel-cleanup-recovery-backup-authorized-removal-revision-1.sh \
    --target slackware-15.0 \
    --execute-authorized-removal \
    --confirm-hostname-fqdn HOSTNAME_FQDN \
    --confirm-rebind-evidence-sha256 SHA256 \
...
This stage performs the revision-1 separately authorized recovery-backup removal after the accepted stable boot rebind. It is
...
while (($#)); do
    case "$1" in
        ...
        --confirm-rebind-evidence-sha256) [[ $# -ge 2 ]] || { usage >&2; exit 2; }; CONFIRM_REBIND_EVIDENCE_SHA256=$2; shift 2 ;;
...
for value in "$TARGET" "$CONFIRM_HOSTNAME_FQDN" "$CONFIRM_REBIND_EVIDENCE_SHA256" "$CONFIRM_ACTIVE_KERNEL" "$CONFIRM_ROLLBACK_KERNEL" "$CONFIRM_RECOVERY_BACKUP_PATH" "$CONFIRM_REMOVAL_TARGET_SHA256" "$CONFIRM_REMOVAL_SCOPE_SHA256"; do
...
[[ $CONFIRM_REBIND_EVIDENCE_SHA256 =~ ^[0-9a-f]{64}$ ]] || { printf 'ERROR: invalid rebind evidence SHA-256.\n' >&2; exit 2; }
...
for required in "$0" "$POLICY" "$ACCEPTED_REBIND" "$STEP111_SCRIPT" "$STEP111_POLICY"; do
...
EVIDENCE_PARENT=/var/tmp/slack-update-acceptance/elilo-oldkernel-cleanup-recovery-backup-authorized-removal-revision-1
...
ACCEPTED_REBIND_SHA256=$(sha_file "$ACCEPTED_REBIND")
STEP111_SCRIPT_SHA256=$(sha_file "$STEP111_SCRIPT")
STEP111_POLICY_SHA256=$(sha_file "$STEP111_POLICY")

EXPECTED_SCRIPT_SHA256=$(json_value "$POLICY" expected_script_sha256)
EXPECTED_ACCEPTED_REBIND_RECORD_SHA256=$(json_value "$POLICY" accepted_rebind_record_sha256)
EXPECTED_ACCEPTED_REBIND_ARCHIVE_SHA256=$(json_value "$POLICY" accepted_rebind_archive_sha256)
EXPECTED_STEP111_SCRIPT_SHA256=$(json_value "$POLICY" accepted_rebind_review_script_sha256)
EXPECTED_STEP111_POLICY_SHA256=$(json_value "$POLICY" accepted_rebind_review_policy_sha256)
...
REMOVAL_SCOPE_SHA256=$(
    printf '%s\n' \
        ...
        "accepted_rebind_archive_sha256=$EXPECTED_ACCEPTED_REBIND_ARCHIVE_SHA256" \
        "accepted_rebind_record_sha256=$ACCEPTED_REBIND_SHA256" \
        "accepted_rebind_review_script_sha256=$STEP111_SCRIPT_SHA256" \
        "accepted_rebind_review_policy_sha256=$STEP111_POLICY_SHA256" \
...
if [[ $SCRIPT_SHA256 == "$EXPECTED_SCRIPT_SHA256" \
    && $ACCEPTED_REBIND_SHA256 == "$EXPECTED_ACCEPTED_REBIND_RECORD_SHA256" \
    && $STEP111_SCRIPT_SHA256 == "$EXPECTED_STEP111_SCRIPT_SHA256" \
    && $STEP111_POLICY_SHA256 == "$EXPECTED_STEP111_POLICY_SHA256" \
    && $(json_value "$ACCEPTED_REBIND" accepted) == true \
    && $(json_value "$ACCEPTED_REBIND" rebind_review_passed) == true \
    && $(json_value "$ACCEPTED_REBIND" stable_boot_identity_verified) == true \
    && $(json_value "$ACCEPTED_REBIND" recovery_backup_retained) == true \
    && $(json_value "$ACCEPTED_REBIND" recovery_backup_removal_reauthorized) == true \
    && $(json_value "$ACCEPTED_REBIND" transient_boot_id_equality_required) == false \
    && $(json_value "$ACCEPTED_REBIND" removal_executed) == false \
    && $(json_value "$ACCEPTED_REBIND" archive_sha256) == "$EXPECTED_ACCEPTED_REBIND_ARCHIVE_SHA256" \
    && $(json_value "$ACCEPTED_REBIND" removal_target_sha256) == "$EXPECTED_REMOVAL_TARGET_SHA256" \
    ...
    && $(json_value "$POLICY" stable_boot_identity_required) == true \
    && $(json_value "$POLICY" transient_boot_id_equality_required) == false \
    && $(json_value "$POLICY" recursive_removal_authorized) == false \
    && $(json_value "$POLICY" glob_removal_authorized) == false ]]; then
    pass "the accepted step-111 boot rebind, exact revision-1 removal code, and exact reviewed target are immutably bound"
else
    fail "the revision-1 destructive removal boundary does not match the accepted step-111 reauthorization"
fi

if [[ $CONFIRM_HOSTNAME_FQDN == "$EXPECTED_HOSTNAME_FQDN" \
    && $CONFIRM_REBIND_EVIDENCE_SHA256 == "$EXPECTED_ACCEPTED_REBIND_ARCHIVE_SHA256" \
    ...
    pass "the explicit host, rebind evidence, kernels, recovery path, removal target, and revision-1 scope confirmations match"
else
    fail "one or more explicit revision-1 destructive-removal confirmations do not match"
fi
...
if [[ $HOSTNAME_FQDN == "$EXPECTED_HOSTNAME_FQDN" \
    ...
    pass "the current host has the exact stable 5.15.209 boot identity required before revision-1 removal"
else
    fail "the host, running kernel, stable BOOT_IMAGE identity, or package database drifted before revision-1 removal"
fi
...
if [[ -s $WORKDIR/packages.before.txt \
    ...
    pass "the complete system and ten-file recovery state was captured before revision-1 destructive removal"
else
...
if cmp -s -- "$WORKDIR/packages.before.txt" "$WORKDIR/packages.after.txt" \
    ...
    pass "package, ELILO, boot, active modules, and rollback-absence state remained unchanged across revision-1 recovery removal"
else
...
if [[ $REMOVAL_EXECUTED == true \
    ...
    pass "the authorized revision-1 recovery-backup removal is committed and the machine is at a safe post-removal boundary"
else
    PAUSE_SAFE=false
    NEXT_STAGE=manual-review-required
...
ARCHIVE="$EVIDENCE_PARENT/${TARGET}-elilo-oldkernel-cleanup-recovery-backup-authorized-removal-revision-1-${TIMESTAMP}.tar.gz"
