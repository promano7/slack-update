...
### Accepted rebound readiness and corrected authorized apply (steps 74-77)
...
The corrected step-77 command performs real package changes. The child acceptance boundary exports `BOOT_CMDLINE_FILE=/proc/cmdline` before invoking the unchanged accepted engine and distinguishes immutable early failure from partial package mutation. The parent validates the short hostname and FQDN independently, then validates the exact readiness record, live pre-transaction boot state, and an authorization scope containing the exact engine, normal-update acceptance, and authorized-wrapper hashes. The nested workflow refreshes metadata and checks the candidate digest again immediately before it invokes the reference engine. Candidate drift blocks before apply. Critical-package authorization is intentionally absent.
...
The corrected focused authorized-apply harness contains 87 checks; the direct-generic and normal-update harnesses contain 56 and 111 checks. The complete prepared step-77 inventory contains 45 suites and 3,095 checks with zero failures; static validation covers 72 shell scripts and 75 JSON files, including both rejected diagnostic records and the code-bound authorization policy.
