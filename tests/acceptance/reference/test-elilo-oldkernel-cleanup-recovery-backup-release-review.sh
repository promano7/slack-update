...
usage() {
    cat <<'USAGE'
Usage:
  test-elilo-oldkernel-cleanup-recovery-backup-release-review.sh \
...
This stage is read-only. It reviews whether the exact retained recovery backup
may be authorized for a later exact-file removal step. It never removes data.
USAGE
}
...
for value in "$TARGET" "$CONFIRM_HOSTNAME_FQDN" "$CONFIRM_POST_REBOOT_EVIDENCE_SHA256" "$CONFIRM_ACTIVE_KERNEL" "$CONFIRM_ROLLBACK_KERNEL" "$CONFIRM_RECOVERY_BACKUP_PATH" "$CONFIRM_RELEASE_SCOPE_SHA256"; do
...
if [[ ! -f $0 || -L $0 || ! -f $POLICY || -L $POLICY || ! -f $ACCEPTED_POST_REBOOT || -L $ACCEPTED_POST_REBOOT ]]; then
    fail "the release-review script, policy, or accepted step-107 record is missing or unsafe"
    printf 'Result: FAIL (%d passes, %d failures, %d skips)\n' "$PASS_COUNT" "$FAIL_COUNT" "$SKIP_COUNT"
    exit 1
fi

TIMESTAMP=$(date -u +%Y%m%dT%H%M%SZ)
EVIDENCE_PARENT=/var/tmp/slack-update-acceptance/elilo-oldkernel-cleanup-recovery-backup-release-review
...
REMOVAL_TARGET_SHA256=$(
    printf '%s\n' \
        "recovery_backup_path=$EXPECTED_RECOVERY_BACKUP_PATH" \
        "boot.tar=$EXPECTED_BOOT_TAR_SHA256" \
        "modules.tar=$EXPECTED_MODULES_TAR_SHA256" \
        "pkgtools.tar=$EXPECTED_PKGTOOLS_TAR_SHA256" \
        'directory_members=boot.tar,modules.tar,pkgtools.tar' \
        | sha256sum | awk '{print $1}'
)
...
if [[ $SCRIPT_SHA256 == "$EXPECTED_SCRIPT_SHA256" \
    ...
    && $(json_value "$POLICY" release_review_authorized) == true \
    && $(json_value "$POLICY" removal_execution_authorized) == false ]]; then
    pass "the accepted step-107 post-reboot verification, exact release-review code, and review policy are bound"
else
    fail "the accepted post-reboot verification or release-review boundary does not match"
fi
...
if [[ $CONFIRM_HOSTNAME_FQDN == "$EXPECTED_HOSTNAME_FQDN" \
    ...
    pass "the explicit host, evidence, kernel, recovery-backup, and release-scope confirmations match"
else
    fail "one or more explicit confirmations do not match the reviewed recovery-backup boundary"
fi
...
if [[ $HOSTNAME_FQDN == "$EXPECTED_HOSTNAME_FQDN" && $RUNNING_KERNEL == "$EXPECTED_ACTIVE_KERNEL" && $CURRENT_BOOT_ID == "$EXPECTED_BOOT_ID" && -n $PKGDB && -d $PKGDB && ! -L $PKGDB ]]; then
    pass "the same successfully verified 5.15.209 boot remains active on the reviewed host"
else
...
if [[ -s $WORKDIR/packages.before.txt && -s $WORKDIR/modules-active-objects.before.sha256 && -s $WORKDIR/boot-state.before.txt && -s $WORKDIR/recovery.before.sha256 && -s $WORKDIR/recovery-directory-members.before.txt && -s $WORKDIR/elilo.conf.before ]]; then
    pass "the package, ELILO, boot, module, and exact recovery-directory state was captured before release review"
else
    fail "one or more required release-review state captures are incomplete"
fi
...
if [[ $(sha_file "$WORKDIR/packages.before.txt") == "$EXPECTED_PACKAGE_SHA256" \
    ...
    pass "the accepted post-cleanup package, active-module, rollback-absence, ELILO, and boot state remains intact"
else
    fail "system state drifted after the accepted post-reboot verification"
fi

EXPECTED_MEMBERS=$'boot.tar\tf\nmodules.tar\tf\npkgtools.tar\tf'
ACTUAL_MEMBERS=$(cat "$WORKDIR/recovery-directory-members.before.txt")
if [[ $ACTUAL_MEMBERS == "$EXPECTED_MEMBERS" ]]; then
    pass "the recovery directory contains exactly the three reviewed regular archives and no extra entries"
else
    fail "the recovery directory contains an unexpected, missing, or non-regular entry"
fi
...
if [[ $(sha_file "$WORKDIR/recovery.before.sha256") == "$EXPECTED_RECOVERY_MANIFEST_SHA256" \
    ...
    pass "all three retained recovery archives match the exact hashes accepted after reboot"
else
    fail "one or more retained recovery archives differ from the accepted recovery snapshot"
fi
...
if [[ $resolved_backup == "$EXPECTED_RECOVERY_BACKUP_PATH" \
    ...
    pass "the reviewed removal target is the exact non-symlink recovery directory under the protected backup parent"
else
    fail "the recovery backup path is not the exact reviewed removal target"
fi
...
capture_recovery_directory_members "$WORKDIR/recovery-directory-members.after.txt" || true
...
if cmp -s -- "$WORKDIR/packages.before.txt" "$WORKDIR/packages.after.txt" \
    ...
    pass "the release review did not modify packages, ELILO, boot artifacts, module objects, or recovery data"
else
    fail "release-review-sensitive system state changed while the read-only review was running"
fi
...
if [[ $FAIL_COUNT -eq 0 ]]; then
    ...
    pass "the exact retained recovery backup is authorized only for a later separately gated exact-file removal"
else
    skip "recovery-backup removal authorization is withheld because the release review has failures"
fi
...
ARCHIVE="$EVIDENCE_PARENT/${TARGET}-elilo-oldkernel-cleanup-recovery-backup-release-review-${TIMESTAMP}.tar.gz"
...
printf 'Next action: preserve the recovery backup and review the failed release boundary.\n'
exit 1
