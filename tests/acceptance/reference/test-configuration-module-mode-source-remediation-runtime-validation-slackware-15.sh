...
CURRENT_REVIEW_POLICY="$REPOSITORY_ROOT/tests/fixtures/reference/acceptance/phase-1/configuration-module-mode-source-remediation-runtime-validation-slackware-current-review-policy.json"
...
CONFIRM_CURRENT_REVIEW_POLICY_SHA256=""

usage() {
    cat <<'USAGE'
Usage:
  test-configuration-module-mode-source-remediation-runtime-validation-slackware-15.sh \
    --target slackware-15.0 \
    --confirm-hostname-fqdn vbox-slack15.vbox-slack15.org \
    --confirm-binding-policy-sha256 SHA256 \
    --confirm-current-review-policy-sha256 SHA256

Run the single authorized non-mutating Slackware 15.0 runtime validation for
Phase 1 optional-module mode source remediation. The target must still match the
accepted UEFI/ELILO generic+initrd boot state. The step-134 Slackware-current
review policy must explicitly release this second execution. An ELILO target
must not become a runnable boot module merely because incidental GRUB tooling
is installed.
USAGE
}
...
while (($#)); do
    case "$1" in
        ...
        --confirm-current-review-policy-sha256) [[ $# -ge 2 ]] || { usage >&2; exit 2; }; CONFIRM_CURRENT_REVIEW_POLICY_SHA256=$2; shift 2 ;;
        -h|--help) usage; exit 0 ;;
...
for value in "$TARGET" "$CONFIRM_HOSTNAME_FQDN" "$CONFIRM_BINDING_POLICY_SHA256" "$CONFIRM_CURRENT_REVIEW_POLICY_SHA256"; do
...
[[ $CONFIRM_CURRENT_REVIEW_POLICY_SHA256 =~ ^[0-9a-f]{64}$ ]] || { printf 'ERROR: invalid current-review policy SHA-256.\n' >&2; exit 2; }
...
for required in "$0" "$SOURCE_FILE" "$TEMPLATE" "$POLICY" "$CURRENT_REVIEW_POLICY" "$ACCEPTED_ELILO_CLOSURE"; do
...
CURRENT_REVIEW_POLICY_SHA256=$(sha_file "$CURRENT_REVIEW_POLICY")
...
EXPECTED_SCRIPT_SHA256=$(json_value "$POLICY" slackware_15.execution_harness_sha256)
EXPECTED_SOURCE_SHA256=$(json_value "$POLICY" source_sha256)
EXPECTED_TEMPLATE_SHA256=$(json_value "$POLICY" template_sha256)
EXPECTED_HOSTNAME_FQDN=$(json_value "$POLICY" slackware_15.hostname_fqdn)
EXPECTED_BOOT_PROFILE=$(json_value "$POLICY" slackware_15.required_boot_profile)
EXPECTED_KERNEL=$(json_value "$POLICY" slackware_15.accepted_kernel)
EXPECTED_BOOT_IMAGE_SUFFIX=$(json_value "$POLICY" slackware_15.required_boot_image_suffix)
EXPECTED_ELILO_SHA256=$(json_value "$POLICY" slackware_15.elilo_conf_sha256)
EXPECTED_ACCEPTED_ELILO_CLOSURE_SHA256=$(json_value "$POLICY" slackware_15.accepted_closure_record_sha256)
EXPECTED_CURRENT_HARNESS_SHA256=$(json_value "$POLICY" slackware_current.execution_harness_sha256)
...
if [[ $CURRENT_REVIEW_POLICY_SHA256 == "$CONFIRM_CURRENT_REVIEW_POLICY_SHA256" ]]; then
    pass "the second execution is bound to the explicitly confirmed Slackware-current review policy"
else
    fail "the Slackware-current review policy SHA-256 does not match the explicit release confirmation"
fi
if python3 - "$CURRENT_REVIEW_POLICY" "$POLICY_SHA256" "$EXPECTED_SOURCE_SHA256" "$EXPECTED_TEMPLATE_SHA256" "$EXPECTED_CURRENT_HARNESS_SHA256" <<'PY_RELEASE'
import json
import re
import sys

path, binding_sha, source_sha, template_sha, current_harness_sha = sys.argv[1:]
with open(path, encoding="utf-8") as handle:
    review = json.load(handle)
assert review["schema"] == 1
assert review["scenario"] == "phase-1-configuration-module-mode-source-remediation-runtime-validation-slackware-current-review"
assert review["review_only"] is True
assert review["binding_policy_sha256"] == binding_sha
assert review["source_sha256"] == source_sha
assert review["template_sha256"] == template_sha
assert review["current_execution_harness_sha256"] == current_harness_sha
assert re.fullmatch(r"[0-9a-f]{64}", review["current_execution_evidence_sha256"])
assert review["current_execution_reviewed"] is True
assert review["current_execution_accepted"] is True
assert review["slackware_15_execution_released"] is True
assert review["slackware_15_execution_authorized"] is True
assert review["slackware_15_machine_execution_limit"] == 1
assert review["reboot_limit"] == 0
assert review["repository_refresh_required"] is False
assert review["package_mutation_authorized"] is False
assert review["boot_mutation_authorized"] is False
assert review["next_stage"] == "phase-1-configuration-module-mode-source-remediation-runtime-validation-slackware-15-execution"
PY_RELEASE
then
    pass "the accepted current-VM evidence review explicitly releases the single Slackware 15.0 execution"
else
    fail "the current-VM review policy does not release the Slackware 15.0 execution"
fi
if [[ $SCRIPT_SHA256 == "$EXPECTED_SCRIPT_SHA256" ]]; then
    pass "the Slackware 15.0 execution harness matches the frozen step-132 SHA-256"
else
    fail "the Slackware 15.0 execution harness does not match the frozen step-132 SHA-256"
fi
...
if [[ $ACCEPTED_ELILO_CLOSURE_SHA256 == "$EXPECTED_ACCEPTED_ELILO_CLOSURE_SHA256" ]]; then
    pass "the established Slackware 15.0 target remains bound to its accepted ELILO closure record"
else
    fail "the accepted Slackware 15.0 ELILO closure record is not the frozen step-132 input"
fi
if [[ $(json_value "$POLICY" machine_execution_authorized) == true \
    ...
    && $(json_value "$POLICY" slackware_15.execution_authorized) == false \
    && $(json_value "$POLICY" slackware_15.authorization_consumable) == false \
    && $(json_value "$POLICY" slackware_15.held_until_current_review) == true \
    && $(json_value "$POLICY" slackware_15.machine_execution_limit) == 1 \
    && $(json_value "$POLICY" reboot_limit) == 0 ]]; then
    pass "the target-binding policy reserves one Slackware 15.0 execution but holds it until current evidence review"
else
    fail "the target-binding policy does not expose the frozen Slackware 15.0 hold-and-release envelope"
fi
...
capture_boot_state() {
    local destination=$1
    {
        ...
        printf 'elilo_conf_sha256=%s\n' "$(sha_file /boot/efi/EFI/Slackware/elilo.conf 2>/dev/null || true)"
        printf 'mkinitrd_config_sha256=%s\n' "$(sha_file /etc/mkinitrd.conf 2>/dev/null || true)"
        printf 'initrd_default_output_sha256=%s\n' "$(sha_file /boot/initrd.gz 2>/dev/null || true)"
...
if [[ $BOOT_IMAGE_COUNT -eq 1 && $BOOT_IMAGE_VALUE == *"$EXPECTED_BOOT_IMAGE_SUFFIX" ]]; then
...
if [[ -f $ELILO_CONF && ! -L $ELILO_CONF && $ELILO_SHA256 == "$EXPECTED_ELILO_SHA256" ]]; then
...
if [[ -f /etc/mkinitrd.conf && ! -L /etc/mkinitrd.conf ]]; then
...
if [[ $BOOT_PROFILE_MATCH == true ]]; then
    ...
    if load_configuration; then
        ...
        if [[ $BOOT_MODE == auto \
            ...
            pass "boot=auto fails closed on the real ELILO initrd-only capability set after the remediation"
        else
...
if [[ $FAIL_COUNT -eq 0 && $BOOT_PROFILE_MATCH == true \
    ...
    NEXT_STAGE=phase-1-configuration-module-mode-source-remediation-runtime-validation-slackware-15-review
...
schema=1
scenario=phase-1-configuration-module-mode-source-remediation-runtime-validation-slackware-15
target=$TARGET
...
current_review_policy_sha256=$CURRENT_REVIEW_POLICY_SHA256
