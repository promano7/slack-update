...
POLICY="$REPOSITORY_ROOT/tests/fixtures/reference/acceptance/phase-1/configuration-module-mode-source-remediation-runtime-validation-slackware-current-rerun-authorization-review-policy.json"
...
Run the single authorized non-mutating Slackware-current runtime validation rerun
for Phase 1 optional-module mode source remediation after the accepted direct-generic
initialization fix. The target must already be in
...
NEXT_STAGE=rerun-manual-review-required
...
for required in "$0" "$SOURCE_FILE" "$TEMPLATE" "$POLICY" "$BINDING_POLICY"; do
...
EXPECTED_BINDING_POLICY_SHA256=$(json_value "$POLICY" accepted_step132_target_binding_policy_sha256)
...
if [[ $POLICY_SHA256 == "$CONFIRM_RERUN_AUTHORIZATION_POLICY_SHA256" ]]; then
    pass "the rerun is bound to the explicitly confirmed step-139 authorization policy"
else
    fail "the step-139 rerun-authorization policy SHA-256 does not match the explicit confirmation"
fi
if [[ $BINDING_POLICY_SHA256 == "$EXPECTED_BINDING_POLICY_SHA256" ]]; then
...
if [[ $SCRIPT_SHA256 == "$EXPECTED_SCRIPT_SHA256" ]]; then
    pass "the Slackware-current rerun harness matches the frozen step-139 SHA-256"
else
    fail "the Slackware-current rerun harness does not match the frozen step-139 SHA-256"
fi
...
if [[ $(json_value "$POLICY" machine_execution_authorized) == true \
    ...
    pass "the step-139 policy authorizes exactly one current-VM rerun and zero reboots"
else
    fail "the step-139 policy does not expose the frozen current-VM rerun authorization envelope"
fi
...
if [[ $FAIL_COUNT -eq 0 && $BOOT_PROFILE_MATCH == true \
    ...
    NEXT_STAGE=phase-1-configuration-module-mode-source-remediation-runtime-validation-slackware-current-rerun-review
    pass "the single authorized Slackware-current rerun completed and is ready for evidence review"
else
...
schema=1
scenario=phase-1-configuration-module-mode-source-remediation-runtime-validation-slackware-current-rerun
target=$TARGET
...
ARCHIVE="$EVIDENCE_PARENT/${TARGET}-configuration-module-mode-source-remediation-runtime-validation-rerun-${TIMESTAMP}.tar.gz"
