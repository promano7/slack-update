...
- Added reusable static ELF helpers for magic-byte filtering, canonical regular-file resolution, `readelf -d` dependency extraction, one-pass `ldconfig -p` cache normalization, exact soname lookup, diagnostics, and post-rebuild reinspection.
...
- ELF scans now build an owner-only normalized library-name cache once per inspection, reject empty cache results, use exact soname matches rather than substring matches, skip non-ELF and non-regular targets before `readelf`, and fail the module when static cache preparation fails.
