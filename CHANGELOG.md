...
- Slackware apply handling now recognizes raw `slackpkg` status `20` from `install-new` and `upgrade-all` as a successful no-package no-op. The raw values remain visible in the final JSON result, while only real failures block secondary modules or produce stable partial status `2`.
...
- Phase 1 real-system acceptance has begun with a reproducible no-updates scenario, expected structured fixtures, evidence capture, and automated harness validation. The first Slackware 15.0 execution on 2026-07-28 passed the real no-updates check, returned stable partial status `2` before invoking `slackpkg`, and preserved both the installed-package database and observed boot state. Evidence SHA-256 `7b712878652ef67de848f6e51d0ea5e889e8d3ab6ea72e69eeb2da7aa87f07a8` proves that 1,594 package records existed; diagnosis identified that `/var/log/packages` was a command-line symlink not followed by the reference enumerator. A second execution reached all Slackware package actions: `update` returned `0`, while `install-new` and `upgrade-all` each returned the documented no-package status `20`; package and observed boot state again remained unchanged, but the reference incorrectly mapped those no-op statuses to stable partial code `2`. Both compatibility issues are now corrected, and a third Slackware 15.0 run remains pending; Slackware-current has not yet been executed.
...
- Package snapshots before and after updates are validated with 38 focused checks. Partial Slackware update handling is validated with 76 focused checks across each `slackpkg` operation, blocked module states, event suppression, provisional JSON, and the successful continuation path. Deterministic SBo target selection is validated with 51 focused checks, dependency-ordered queue construction is validated with 32 focused checks, SBo build-option preservation is validated with 57 focused checks, personal queue isolation is validated with 92 focused checks, static ELF non-execution is validated with 41 focused checks, architecture-specific ELF resolution is validated with 43 focused checks, installed-kernel initrd validation is validated with 69 focused checks, GRUB blocking after initrd failure is validated with 55 focused checks, staged GRUB validation plus atomic replacement is validated with 87 focused checks, interruption cleanup is validated with 56 focused checks, non-zero error propagation is validated with 137 focused checks, minimal cron execution is validated with 79 focused checks, and the no-updates acceptance harness is validated with 60 focused checks. The next development task is to rerun and review this first scenario on real Slackware 15.0, then execute it on Slackware-current.
- The sixteen focused reference suites currently total 1,044 checks, all passing in the validation environment.
...
- Partial-update tests confirm that real failures from `update`, `install-new`, or `upgrade-all` still permit final snapshot capture but block every secondary module, suppress their start events, and report `blocked` states in provisional JSON. They also confirm that raw status `20` from `install-new` and `upgrade-all` is a successful no-package result, remains observable in structured output, and preserves stable process code `0`.
...
- The reference script has not yet passed the Phase 1 acceptance matrix on Slackware 15.0 or Slackware-current. Two Slackware 15.0 no-updates runs exposed and isolated compatibility issues: the `/var/log/packages` command-line symlink and the successful `slackpkg` no-package status `20`. Both are corrected; a clean third run and evidence review remain pending.
