...
### Phase 1 steps 75-77: corrected explicitly authorized Slackware-current application
...
The step-77 normal-update acceptance boundary exports that mutable path as `/proc/cmdline` before invoking the unchanged accepted engine. Its acceptance logic now records an early failure with identical package snapshots as unchanged state, while any failed apply with package drift is explicitly classified as partial. The authorization scope includes the exact reference-engine, normal-update acceptance, and authorized-wrapper SHA-256 values, so every code correction requires a new explicit authorization digest. The wrapper still validates the accepted readiness record and complete live 6.18.40 boot baseline, then invokes `test-normal-update.sh --execute-apply` with the verified FQDN. The child refreshes Slackpkg metadata and reconstructs the candidate set again immediately before package installation. Any new, removed, or replaced candidate changes the digest and blocks before the reference apply workflow is called.
...
The corrected step-77 authorized-apply harness contains 87 checks; the direct-generic and normal-update harnesses contain 56 and 111 checks. The complete step-77 inventory contains 45 suites and 3,095 checks with zero failures. Static validation covers 72 shell scripts and 75 JSON files.
