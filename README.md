...
Step 47 restarts exact-package inspection for `6.18.42`. The preflight now requires the accepted 69-candidate record, the accepted corrected boot-layout record, and the accepted chain-restart record to agree on the digest, running kernel, target kernel, deferred target-image boundary, and denied apply state. Only then may it download `kernel-generic-6.18.42-x86_64-1.txz`, validate archive paths, prove `boot/vmlinuz-6.18.42` plus `/lib/modules/6.18.42`, inspect but never execute `doinst.sh`, and generate a current-system GRUB baseline solely inside private evidence. The real system remains unchanged and both `apply_ready` and `apply_authorized` remain false.
...
- [ ] Execute and document the Phase 1 acceptance matrix on Slackware 15.0 and Slackware-current.
  ...
  - [ ] Repeat the corrected target-bound boot restart, then package, GenInitrd, DKMS, command, and GRUB-ownership evidence for `6.18.42`.
