...
The wrapper must validate the accepted 6.18.40 live baseline before starting. The real acceptance boundary must initialize every mutable live boot-probe input, including `BOOT_CMDLINE_FILE=/proc/cmdline`, before invoking the accepted engine with `set -u`. Its child normal-update acceptance workflow must refresh metadata and compare the complete candidate digest again before calling the reference apply engine. Do not add `--allow-critical-update`; this reviewed transaction contains zero critical candidates. Any candidate drift must block before package mutation. An early engine failure with identical before/after package snapshots is immutable failure evidence, while package drift after a failed apply is a partial transaction requiring manual recovery.
