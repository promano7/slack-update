...
out=$2
mkdir -p -- "$out"
cat > "$out/phase-1-kernel-package-edge-local-source-construction-target-artifact-staging-authorization-review-policy.json" <<'POLICY208'
{
  ...
  "authorization": {
    ...
    "stager_execution_authorized": true,
    "target_artifact_copy_authorized": true,
    "target_artifact_staging_result_review_required_after_execution": true
  },
  ...
  "helper_path": "tools/reference/phase-1-kernel-package-edge-local-source-construction-target-artifact-staging-authorization-review.sh",
  "machine_action_required": true,
  "next_stage": "phase-1-kernel-package-edge-local-source-construction-target-artifact-staging-result-review",
  "pause_safe": false,
  "review_only": false,
  "revision": "single-execution-target-artifact-local-staging-authorization",
  "scenario": "phase-1-kernel-package-edge-local-source-construction-target-artifact-staging-authorization-review",
  "schema": 1,
...
POLICY208
cat > "$out/phase-1-kernel-package-edge-local-source-construction-target-artifact-staging-authorization-review.tsv" <<'RECORD208'
step	208
revision	single-execution-target-artifact-local-staging-authorization
fresh_target_binding_state	frozen
builder_implementation_state	frozen
target_artifact_staging_state	authorized-awaiting-single-execution
stager_path	tools/reference/phase-1-kernel-package-edge-target-artifact-stage.sh
stager_sha256	a57303868eff7ee5ad8bb597d0dff52dec74887732fe590c40bae0edeb7d05eb
execution_target	slackware-current-target-vm
execution_acknowledgement	--stage-target-artifact
...
acceptance_root_must_not_preexist	yes
transport_source_preserved	yes
transport_source_symlink_allowed	no
production_paths_overridable	no
runtime_network_access_allowed	no
single_execution_authorization	yes
controller_transport_copy_authorized	yes
target_artifact_copy_authorized	yes
stager_execution_authorized	yes
